...
export const MoveToPenModal: React.FC<MoveToPenModalProps> = ({
  ...
  return (
    ...
    >
      <div className="bg-white w-full max-w-3xl rounded-[2.5rem] shadow-2xl border border-slate-100 overflow-hidden flex flex-col my-auto max-h-[92vh]">
        {/* Modal Header */}
        <div className="px-6 sm:px-8 pt-6 pb-4 border-b border-slate-100 flex items-center justify-between bg-gradient-to-r from-slate-50 via-white to-blue-50/30">
          <div className="flex items-center gap-3.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 font-medium mt-0.5">
                Relocate single or multiple animals into customized pens defined in Settings.
              </p>
            </div>
          </div>
          ...
          <button
            ...
            type="button"
            onClick={onClose}
            className="p-2.5 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-2xl transition-all cursor-pointer"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 sm:p-8 overflow-y-auto space-y-6 flex-1">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2 p-1.5 bg-slate-100 rounded-2xl">
              <button
                ...
                className={`flex items-center justify-center gap-2 py-3 rounded-xl font-black text-xs uppercase tracking-wider transition-all cursor-pointer ${
                  selectionMode === 'single'
                    ? 'bg-white text-blue-600 shadow-sm'
                    : 'text-slate-500 hover:text-slate-700'
                }`}
              >
                <User className="w-4 h-4" /> Single Animal
              </button>
              <button
                type="button"
              ...
              >
                <Users className="w-4 h-4" /> Multiple Animals (Flock / Batch)
              </button>
            </div>
          </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between px-1">
              ...
              <span className="text-[10px] font-bold text-slate-400">
                {groupsList.length} pens configured in Settings
              </span>
            </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between gap-2">
                      <span className={`text-xs font-black tracking-tight ${isSelected ? 'text-blue-900' : 'text-slate-700'}`}>
                        {group}
                      </span>
                    ...
                    <div className="flex items-center gap-1 mt-2 text-[10px] font-bold text-slate-400">
                      ...
                      <span>{count} {count === 1 ? 'cow' : 'cows'} currently</span>
                    </div>
                  </button>
                );
              })}
          ...
          <div className="space-y-3">
            ...
            <div className="max-h-56 overflow-y-auto border border-slate-200/80 rounded-2xl p-2.5 bg-slate-50/50 space-y-1.5">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            {animal.name && (
                              <span className="text-[11px] font-bold text-slate-500">({animal.name})</span>
                            )}
                            <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded-md border ${getStatusColorBadge(animal.status)}`}>
                      ...
                      <div className="flex items-center gap-2">
                        {isAlreadyInTarget ? (
                          <span className="text-[9px] font-bold text-amber-700 bg-amber-50 border border-amber-200 px-2 py-0.5 rounded-md">
                            Already in {targetGroup}
                          </span>
                        ) : (
                          <span className="text-[10px] font-bold text-slate-400">
                            ➔ {targetGroup || 'Choose target'}
                          </span>
                        )}
                      </div>
                    </div>
            ...
            {selectionMode === 'multiple' && selectedMultipleIds.length > 0 && (
              <div className="p-3 bg-blue-50/70 rounded-2xl border border-blue-100">
                <div className="flex items-center justify-between text-[11px] font-black text-blue-900 mb-2">
                ...
                <div className="flex flex-wrap gap-1.5 max-h-24 overflow-y-auto">
                  {selectedAnimalsList.map(a => (
                    <span
                      key={a.id}
                      className="inline-flex items-center gap-1 bg-white border border-blue-200 px-2.5 py-1 rounded-xl text-[11px] font-black text-blue-800 shadow-2xs"
                    >
                      {a.tag}
                      <button
                        type="button"
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block px-1">
              4. Transfer Reason / Log Note (Optional)
            </label>
            ...
            <input
              type="text"
              placeholder="e.g., Moved for scheduled estrus sync or calving preparation..."
              value={transferReason}
              onChange={(e) => setTransferReason(e.target.value)}
              className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-xs font-bold text-slate-700 outline-none focus:ring-2 focus:ring-blue-500/20"
            />
          </div>

          {/* Transfer Summary Preview */}
          {activeSelectedIds.length > 0 && targetGroup && (
            <div className="p-4 bg-gradient-to-r from-emerald-50 to-teal-50 rounded-2xl border border-emerald-200/80 flex items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs font-black text-emerald-900">
                    Ready to move {activeSelectedIds.length} {activeSelectedIds.length === 1 ? 'animal' : 'animals'}
                  </p>
                  <p className="text-[11px] text-emerald-700 font-medium">
                    Destination: <strong className="font-black text-emerald-900">{targetGroup}</strong> Pen
                  </p>
                </div>
              </div>
              <span className="text-[10px] font-black uppercase tracking-wider bg-emerald-200/80 text-emerald-900 px-3 py-1 rounded-full">
          ...
          <div className="pt-2 flex items-center justify-end gap-3 border-t border-slate-100">
            <button
              type="button"
              onClick={onClose}
              className="px-6 py-3.5 bg-slate-100 text-slate-700 rounded-2xl font-black text-xs uppercase tracking-wider hover:bg-slate-200 transition-all cursor-pointer"
            >
              Cancel
            </button>
            <button
              ...
              className="px-8 py-3.5 bg-blue-600 text-white rounded-2xl font-black text-xs uppercase tracking-wider hover:bg-blue-700 transition-all shadow-lg shadow-blue-200 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer flex items-center gap-2"
            >
